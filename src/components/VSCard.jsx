    ...
    <div className={"rounded-3xl overflow-hidden scale-in relative border " + (hh ? "border-red-500/50" : hm ? "border-boxing-goldDim/50" : "border-white/10")} style={{ background: "linear-gradient(180deg,#120e14,#0b090c)" }}>
      <div className="flex items-center justify-between px-4 py-2 bg-black/30 border-b border-white/5">
        <div className="flex items-center gap-2.5 min-w-0">
          <span className="text-boxing-muted flex-shrink-0" style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: "14px", letterSpacing: "0.2em" }}>PELEA {matchup.roundNumber}</span>
          {forced && <span className="text-[14px] font-bold px-2 py-0.5 rounded-full bg-red-900/50 border border-red-500/60 text-red-300 tracking-widest uppercase flex-shrink-0">Forzada</span>}
          <span className="text-[14px] font-bold px-2.5 py-0.5 rounded-full border border-boxing-goldDim/40 text-boxing-goldFight tracking-widest uppercase truncate" style={{ boxShadow: "inset 0 0 10px rgba(200,160,74,0.12)" }}>{rc?.label} · dif. {wd}kg</span>
        </div>
      ...
      <div className="vsx-body grid grid-cols-[1fr_auto_1fr] items-center px-4 py-5 gap-3">
        <div className="relative z-10 flex flex-col gap-1 min-w-0 text-right">
          <span className="text-[14px] font-bold tracking-[0.2em] uppercase" style={{ color: "rgba(232,80,95,0.85)" }}>Esquina roja</span>
          ...
          <div className="text-[14px] text-boxing-muted truncate">{r.gym} · {r.weightKg}kg · {r.age}a</div>
          <div className="text-[14px] font-semibold tracking-widest uppercase mt-0.5" style={{ color: re?.color }}>{r.fightCount}p · {re?.label}</div>
        </div>
        ...
        <div className="relative z-10 flex flex-col gap-1 min-w-0 text-left">
          <span className="text-[14px] font-bold tracking-[0.2em] uppercase" style={{ color: "rgba(96,145,246,0.9)" }}>Esquina azul</span>
          ...
          <div className="text-[14px] text-boxing-muted truncate">{b.weightKg}kg · {b.age}a · {b.gym}</div>
          <div className="text-[14px] font-semibold tracking-widest uppercase mt-0.5" style={{ color: be?.color }}>{b.fightCount}p · {be?.label}</div>
        </div>
      </div>
      ...
      {forced && <div className="border-t border-red-500/30 px-4 py-2.5 bg-red-900/15">
        {forcedReasons.length > 0
          ? <p className="text-[14px] leading-snug"><span className="text-red-400 font-bold uppercase tracking-wide">🔴 Forzada — para cumplir la norma faltaría: </span><span className="text-red-300/90">{forcedReasons.map(r => `(${r})`).join("; ")}.</span></p>
          : <p className="text-[14px] leading-snug text-green-400">✓ Forzada, pero este cruce sí cumple la norma (no le falta ninguna condición).</p>}
      </div>}
      {warnings.length > 0 && <div className="border-t border-white/5 px-4 py-2 space-y-1 bg-black/20">{warnings.map((w, i) => <div key={i} className="flex items-center gap-1.5 text-[14px]"><span>{w.severity === "high" ? "\u{1F534}" : w.severity === "medium" ? "\u{1F7E1}" : "\u{1F7E2}"}</span><span className={w.severity === "high" ? "text-red-400" : w.severity === "medium" ? "text-boxing-goldFight" : "text-boxing-muted"}>{w.message}</span></div>)}</div>}
      {onNotaChange && <div className="border-t border-white/5 px-4 py-2">
        <input type="text" value={matchup.nota || ""} onChange={e => onNotaChange(matchup.id, e.target.value)} placeholder="Nota (ej: Super 4, 5 peleas, exhibición...)" maxLength={40} className="w-full bg-transparent text-boxing-cream placeholder-boxing-muted text-sm rounded-lg focus:outline-none focus:shadow-[0_0_0_1px_rgba(37,99,235,0.55),0_0_14px_rgba(37,99,235,0.3)] px-1 py-0.5 transition-shadow" />
      </div>}
    </div>
  );
}
