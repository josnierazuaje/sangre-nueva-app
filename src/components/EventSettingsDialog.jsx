...
export default function EventSettingsDialog({ label, dates, isOwner, onSave, onClose }) {
  ...
  function guardar() {
    ...
    onSave({ label: titulo.trim(), dates: { semis, final } });
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-md bg-boxing-panel border border-boxing-goldDim/50 rounded-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="px-4 py-3 border-b border-boxing-line">
          ...
          <p className="text-[14px] text-boxing-muted">El título y las fechas que salen en la cartelera y en las llaves</p>
        </div>
        ...
        <div className="p-4 space-y-4">
          ...
          {!isOwner && <p className="text-[14px] text-boxing-muted">Solo el organizador puede cambiar las fechas. El título sí lo puedes editar.</p>}
